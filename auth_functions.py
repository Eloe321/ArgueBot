    ...
    if 'user_info' not in st.session_state:
        ...
        if auth_token:
            try:
                ...
                user_info = get_account_info(auth_token)["users"][0]
...
def sign_in(email:str, password:str) -> None:
    try:
        ...
        auth_response = sign_in_with_email_and_password(email, password)
        ...
        # Save user info to session state and rerun
            ...
            st.session_state.user_info = user_info
             ...
             # Save auth info to cookie
            cookie_manager = get_cookie_manager()
            cookie_manager.set('auth_token', id_token, expires_at=datetime.now() + timedelta(days=7))
